...
using System.IO;
// [System.Web.Extension] should be added from [Reference]
...
namespace Atom
{
    class Program
    {
        ...
        public static List<Atom> elements = new List<Atom>();
        ...
        static void Main(string[] args)
        {
            ...
            XMLSerializationCollectionObject();

            Console.WriteLine("\n----------\n");

            //5 - Read the above file and display the all the items
            XMLDeSerializationCollectionObject();

            Console.WriteLine("\n----------\n");

            //6 - Serialize the first item using json format and save to a suitable file
            JASONSerializationOneObject(elements.First(), "AtomJasonOne.jason");

            Console.WriteLine("\n----------\n");

            //7 - Read the above file and display the item
            JASONDeSerializationOneObject(elements.First(), "AtomJasonOne.jason");

            Console.WriteLine("\n----------\n");

            //8 - Serialize the entire collection using json format and save to a suitable file
            JASONSerializationCollectionObject(elements, "AtomJasonCollection.jason");

            Console.WriteLine("\n----------\n");

            //9 - Read the above file and display all of the items
            JASONDeSerializationCollectionObject(elements, "AtomJasonCollection.jason");
        }

        static void XMLSerializationOneObject()
        {
            XmlSerializer xmlSerializerOneObject = new XmlSerializer(typeof(Atom));
            TextWriter xmlWriter = new StreamWriter("AtomXMLOne.xml");
            xmlSerializerOneObject.Serialize(xmlWriter, elements.First());
            xmlWriter.Close();

            Console.WriteLine("AtomXMLOne.xml file was created.");
        }

        static void XMLDeSerializationOneObject()
        {
            XmlSerializer xmlDeSerializerOneObject = new XmlSerializer(typeof(Atom));
            TextReader xmlReader = new StreamReader("AtomXMLOne.xml");
            Atom atom = (Atom)xmlDeSerializerOneObject.Deserialize(xmlReader);
            Console.WriteLine(atom);
            xmlReader.Close();
        }

        static void XMLSerializationCollectionObject()
        {
            XmlSerializer xmlSerializerCollectionObject = new XmlSerializer(typeof(List<Atom>));
            TextWriter xmlWriter = new StreamWriter("AtomXMLCollection.xml");
            xmlSerializerCollectionObject.Serialize(xmlWriter, elements);
            xmlWriter.Close();

            Console.WriteLine("AtomXMLCollection.xml file was created.");
        }

        static void XMLDeSerializationCollectionObject()
        {
            XmlSerializer xmlDeSerializerCollectionObject = new XmlSerializer(typeof(List<Atom>));
            TextReader xmlReader = new StreamReader("AtomXMLCollection.xml");
            List<Atom> atoms = (List<Atom>)xmlDeSerializerCollectionObject.Deserialize(xmlReader);
            foreach(Atom x in atoms)
            {
                Console.WriteLine(x);
            }
            xmlReader.Close();
        ...
        static void JASONSerializationOneObject(Atom atom, string fileName)
        {
            using (TextWriter jasonWriter = new StreamWriter(fileName))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                jasonWriter.Write(serializer.Serialize(atom));
                jasonWriter.Close();

                Console.WriteLine("AtomJasonOne.jason file was created.");
            }
        }

        static void JASONDeSerializationOneObject(Atom atom, string fileName)
        {
            using (TextReader jasonReader = new StreamReader(fileName))
            {
                JavaScriptSerializer deSerializer = new JavaScriptSerializer();
                Console.WriteLine(deSerializer.Deserialize<Atom>(jasonReader.ReadToEnd()));
                jasonReader.Close();
            }
        }

        static void JASONSerializationCollectionObject(List<Atom> atom, string fileName)
        {
            using (TextWriter jasonWriter = new StreamWriter(fileName))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                jasonWriter.Write(serializer.Serialize(elements));
                jasonWriter.Close();

                Console.WriteLine("AtomJasonCollection.jason file was created.");
            }
        }

        static void JASONDeSerializationCollectionObject(List<Atom> atom, string fileName)
        {
            using (TextReader jasonReader = new StreamReader(fileName))
            {
                JavaScriptSerializer deSerializer = new JavaScriptSerializer();
                List<Atom> atomList = deSerializer.Deserialize<List<Atom>>(jasonReader.ReadToEnd());
